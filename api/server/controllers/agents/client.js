...
class AgentClient extends BaseClient {
  ...
  async buildMessages(messages, parentMessageId, _buildOptions, opts) {
     ...
     * This includes: project context, file context (latest message), augmented prompt (RAG),
     * and memory context (Mem0 semantic search) — applied to every agent in the run.
     */
     ...
     *  Mem0 has no keyed/unkeyed or per-agent-partition split (those are
     *  upstream native-memory concepts) — every qualifying agent gets the
     *  same Mem0 context. Shim the partition helpers the shared per-agent
     *  loop below expects so it works unchanged; the Mem0 query runs once. */
    ...
    const getAgentPartitionMemories = async () =>
      memoryResults ? { withKeys: memoryResults, withoutKeys: memoryResults } : undefined;
    /** Mem0 shim: no separate canonical/PII inspection path — memories flow
     *  through buildMemoryContext, so the model-bound PII loop is a no-op. */
    const getCanonicalAgentMemories = async () => undefined;
   ...
   */
  async getEventActorMemorySnapshots(_agents) {
    const primary = await this.getSharedMemoryContext();
    if (!primary) {
      return [];
    }
    /** Mem0 shim: Mem0 has no per-agent partitions (that is an upstream native-memory
     *  concept), so every agent shares one scope and the event-actor snapshot is a
     *  single shared entry rather than a per-agent map. */
    return [{ scope: 'shared', withKeys: primary, withoutKeys: primary }];
  }
