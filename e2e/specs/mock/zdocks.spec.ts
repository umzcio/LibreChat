 ...
 */
async function createZdock(page: Page, name: string): Promise<string> {
  await page.goto('/zdocks', { timeout: 10000 });
  ...
  const zdockId = new URL(page.url()).pathname.split('/projects/')[1];
  expect(zdockId).toBeTruthy();
  return zdockId;
}
...
test.describe('chat projects', () => {
  ...
  test('starts a project-scoped chat and persists it under the project', async ({ page }) => {
    ...
    const name = uniqueName('E2E Project');
    const zdockId = await createZdock(page, name);
    ...
    await page.goto(`/c/new?zdockId=${zdockId}`, { timeout: 10000 });
    ...
    await expect(
      page.getByTestId(`project-chats-${zdockId}`).getByTestId('convo-item').first(),
    ).toBeVisible();
  ...
  test('removes the project scope via the chip ×', async ({ page }) => {
    ...
    await expect(page).toHaveURL((url) => !url.searchParams.has('zdockId'));
  });
  ...
  test('switches the project via the chip combobox', async ({ page }) => {
    ...
    await expect(page).toHaveURL(new RegExp(`zdockId=${projectIdB}`));
